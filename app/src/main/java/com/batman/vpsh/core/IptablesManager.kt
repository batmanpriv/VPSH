...
class IptablesManager(private val shell: RootShell) {
    ...
    fun applyClientLimit(hotspotIface: String, clientIp: String, kbps: Int): Boolean {
        ...
        shell.run("tc filter del dev $hotspotIface parent $HTB_ROOT_HANDLE protocol ip prio 1 u32 match ip dst $clientIp/32 2>/dev/null")
        val filterRes = shell.run(
            "tc filter add dev $hotspotIface parent $HTB_ROOT_HANDLE protocol ip prio 1 u32 " +
