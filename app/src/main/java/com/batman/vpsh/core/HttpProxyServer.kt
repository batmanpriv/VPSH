...
class HttpProxyServer(
    ...
    private fun handleClient(client: Socket, limiter: RateLimiter?, clientIp: String) {
        try {
            ...
            if (requiresAuth() && !isAuthorized(headers, clientIp)) {
            ...
            if (method.equals("CONNECT", ignoreCase = true)) {
                handleConnect(client, target, limiter, clientIp)
            } else {
                handlePlainHttp(client, method, target, headers, input, limiter, clientIp)
            }
        } catch (_: Exception) {
            try { client.close() } catch (_: Exception) { }
        }
    ...
    private fun isAuthorized(headers: Map<String, String>, clientIp: String): Boolean {
        ...
        return authChecker(decoded.substring(0, sep), decoded.substring(sep + 1), clientIp)
    }
    ...
    private fun handleConnect(client: Socket, target: String, limiter: RateLimiter?, clientIp: String) {
