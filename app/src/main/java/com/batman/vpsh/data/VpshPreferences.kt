...
data class VpshSettings(
    ...
    val enablePac: Boolean = true,
    val pacPort: Int = 8199,
    val autoStartOnBoot: Boolean = false,
    val gameModeEnabled: Boolean = false,
    val disableIpaOffload: Boolean = false,

    val apAutoCreate: Boolean = false,
    val apSsid: String = "VPSH",
    val apPassword: String = "",

    val enableShadowsocks: Boolean = false,
    val shadowsocksPort: Int = 8488,
...
class VpshPreferences(private val context: Context) {
    ...
    private object Keys {
        ...
        val ENABLE_PAC = booleanPreferencesKey("enable_pac")
        ...
        val DISABLE_IPA_OFFLOAD = booleanPreferencesKey("disable_ipa_offload")
        val AP_AUTO_CREATE = booleanPreferencesKey("ap_auto_create")
        val AP_SSID = stringPreferencesKey("ap_ssid")
        val AP_PASSWORD = stringPreferencesKey("ap_password")
        val SS_ENABLE = booleanPreferencesKey("ss_enable")
        val SS_PORT = intPreferencesKey("ss_port")
    ...
    val settingsFlow: Flow<VpshSettings> = context.dataStore.data.map { p ->
        VpshSettings(
            ...
            enablePac = p[Keys.ENABLE_PAC] ?: true,
            pacPort = p[Keys.PAC_PORT] ?: 8199,
            autoStartOnBoot = p[Keys.AUTO_START_BOOT] ?: false,
            gameModeEnabled = p[Keys.GAME_MODE] ?: false,
            disableIpaOffload = p[Keys.DISABLE_IPA_OFFLOAD] ?: false,
            apAutoCreate = p[Keys.AP_AUTO_CREATE] ?: false,
            apSsid = p[Keys.AP_SSID] ?: "VPSH",
            apPassword = p[Keys.AP_PASSWORD] ?: "",
            enableShadowsocks = p[Keys.SS_ENABLE] ?: false,
            shadowsocksPort = p[Keys.SS_PORT] ?: 8488,
            shadowsocksPassword = p[Keys.SS_PASSWORD] ?: "",
    ...
    suspend fun update(transform: (VpshSettings) -> VpshSettings) {
        ...
        context.dataStore.edit { p ->
            ...
            p[Keys.ENABLE_PAC] = next.enablePac
            p[Keys.PAC_PORT] = next.pacPort
            p[Keys.AUTO_START_BOOT] = next.autoStartOnBoot
            p[Keys.GAME_MODE] = next.gameModeEnabled
            p[Keys.DISABLE_IPA_OFFLOAD] = next.disableIpaOffload
            p[Keys.AP_AUTO_CREATE] = next.apAutoCreate
            p[Keys.AP_SSID] = next.apSsid
            p[Keys.AP_PASSWORD] = next.apPassword
            p[Keys.SS_ENABLE] = next.enableShadowsocks
            p[Keys.SS_PORT] = next.shadowsocksPort
            p[Keys.SS_PASSWORD] = next.shadowsocksPassword
    ...
    suspend fun wasRunning(): Boolean = context.dataStore.data.map { it[Keys.WAS_RUNNING] ?: false }.first()
}
