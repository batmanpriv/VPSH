...
import com.batman.vpsh.ui.screens.NetworkScreen
import com.batman.vpsh.ui.screens.SettingsScreen
import com.batman.vpsh.ui.screens.UpdateScreen
...
import com.batman.vpsh.viewmodel.NetworkViewModel
import com.batman.vpsh.viewmodel.UpdateViewModel
...
private val NAV_DESTINATIONS = listOf(
    ...
    NavDestination(5, Icons.Filled.Router, R.string.nav_network),
    NavDestination(6, Icons.Filled.Info, R.string.nav_about),
    NavDestination(7, Icons.Filled.SystemUpdate, R.string.nav_update)
)
...
fun VpshApp(
    ...
    val netIsRooted by networkViewModel.isRooted.collectAsState()
    val netInterfaces by networkViewModel.interfaces.collectAsState()
    val netGateway by networkViewModel.gateway.collectAsState()
    val netDns by networkViewModel.dns.collectAsState()
    val netTable by networkViewModel.table.collectAsState()
    val netChains by networkViewModel.chains.collectAsState()
    val netIpRules by networkViewModel.ipRules.collectAsState()
    val netIpRoutes by networkViewModel.ipRoutes.collectAsState()
    val netBusy by networkViewModel.busy.collectAsState()
    val netMessage by networkViewModel.message.collectAsState()
            ...
            Surface(modifier = Modifier.padding(padding)) {
                when (tab) {
                    ...
                    3 -> SettingsScreen(
                        ...
                        apActive = state.apSelfManaged,
                        onTestAccessPoint = { ssid, password, onResult -> viewModel.testAccessPoint(ssid, password, onResult) },
                        onStopAccessPoint = { viewModel.stopTestAccessPoint() }
                    )
                    4 -> LogsScreen(logs = state.logs, onHealthCheck = { viewModel.runHealthCheck() })
                    5 -> NetworkScreen(
                        isRooted = netIsRooted,
                        ...
                        chains = netChains,
                        ipRules = netIpRules,
                        ipRoutes = netIpRoutes,
                        busy = netBusy,
                        message = netMessage,
                        onRefresh = { networkViewModel.refreshAll() },
                        onSetTable = { t -> networkViewModel.setTable(t) },
                        onAppendRule = { chain, args -> networkViewModel.appendRule(chain, args) },
                        onRunRawIptables = { args -> networkViewModel.runRawIptables(args) },
                        onDeleteRule = { chain, line -> networkViewModel.deleteRule(chain, line) },
                        onRunRawIp = { args -> networkViewModel.runRawIp(args) },
                        onDismissMessage = { networkViewModel.clearMessage() }
                    )
                    6 -> AboutScreen()
                    else -> UpdateScreen(
                        state = updateState,
                        ...
                        onOpenReleasePage = { url -> updateViewModel.openReleasePage(url) }
                    )
                }
            }
        }
